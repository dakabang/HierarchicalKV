...
void test_find(size_t max_hbm_for_vectors, size_t max_bucket_size,
  ...
  constexpr uint64_t TEST_TIMES = 2;
  ...
  for (int i = 0; i < TEST_TIMES; ++i) {
    ...
    if (insert_num == 0) {
      ...
      std::vector<K> keys(KEY_NUM);
      CUDA_CHECK(cudaMemcpy(keys.data(), d_keys, KEY_NUM * sizeof(K),
                            cudaMemcpyDeviceToHost));
      // Check missed
      ...
      for (uint64_t j = 0; j < KEY_NUM; ++j) {
        if (founds[j]) {
          ASSERT_EQ(keys[j], h_keys[j]);
          ASSERT_EQ(h_scores[j], keys[j]);
